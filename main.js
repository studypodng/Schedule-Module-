...
const STORAGE_KEYS = ['estudy_profiles','estudy_stats','estudy_conversations','estudy_messages','estudy_offers'];
...
function seedDemoData() {
  ...
  const offers = [
    {
      ...
      summary: 'Weekly 2hr sessions covering integrals, derivatives and exam prep.',
  ...
  const messages = [
    ...
    },
    {
      ...
      senderRole: 'tutor',
      type: 'text',
      content: 'Hello Alex! Absolutely, we can schedule 2hr sessions twice a week. What times work for you?',
...
function ensureSeed() {
  const hasAny = STORAGE_KEYS.some(k => {
    ...
    try { const arr = JSON.parse(v||'[]'); return Array.isArray(arr) && arr.length>0; } catch { return false; }
  });
...
export async function initApp(role) {
  ...
  function closeMobile() { overlay.classList.remove('open'); }
  function openMobile() { overlay.classList.add('open'); }
  ...
  function updateGridCollapse() {
    ...
    mainEl.classList.toggle('both-collapsed', s.leftCollapsed && s.rightCollapsed);
  }
  ...
  const mobileNavComp = createLeftNav({ store, conversationService: ConversationService, profileService: ProfileService, offerService: OfferService });
  ...
  window.estudyReset = () => { seedDemoData(); location.reload(); };
}
