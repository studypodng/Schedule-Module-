...
export function createRightPanel({ store, profileService }) {
  ...
  function readStats() {
    try { return JSON.parse(localStorage.getItem('estudy_stats') || '[]'); } catch { return []; }
  }
  ...
  function render() {
    ...
    toggle.setAttribute('aria-label', isCollapsed ? 'Expand details' : 'Collapse details');
    toggle.textContent = isCollapsed ? '‹' : '›';
    toggle.addEventListener('click', () => {
      store.setState({ rightCollapsed: !store.getState().rightCollapsed });
      render();
      document.dispatchEvent(new CustomEvent('rightCollapseChanged', { detail: !isCollapsed }));
    });
    ...
    if (otherProfile.role === 'tutor') subj.textContent = otherProfile.specialty || '';
    ...
    statTitle.textContent = otherProfile.role === 'student' ? 'Learning stats' : 'Teaching stats';
  ...
  async function updateActiveConversation(conv) {
    if (!conv) { otherProfile = null; otherStats = null; render(); return; }
    const state = store.getState();
    ...
    mount(el) {
      ...
      store.subscribe('profiles:changed', () => {
        ...
        if (s.activeConversationId) {
          ...
          if (otherProfile) updateActiveConversation({ tutorId: otherProfile.role==='tutor'? otherProfile.id : null, studentId: otherProfile.role==='student'? otherProfile.id : null });
        }
      });
      render();
    }
