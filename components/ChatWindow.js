...
export function createChatWindow({ store, chatService, offerService, conversationService, profileService }) {
  let container = null;
  let messages = [];
  let activeConv = null;
  ...
  function render() {
    ...
    if (state.currentRole === 'tutor') {
      if (pendingOffer) {
        ...
        info.textContent = 'You have a pending offer. Edit it or wait for response.';
  ...
  async function renderSingleMessage(msg, placeholder) {
    ...
    const avatarUrl = mine ? (await profileService.getProfile(state.currentUserId))?.profilePicUrl : otherProfile?.profilePicUrl;

    let node;
    if (msg.type === 'offer') {
      node = await offerRenderer(msg, { mine, avatarUrl, currentRole: state.currentRole, onUpdate: async () => {
        await refreshMessages();
        store.publish('conversations:changed', null);
      }});
    } else {
      node = bubbleRenderer(msg, { mine, avatarUrl, onDelete: refreshMessages });
  ...
  return {
    mount(el) {
      container = el;
      store.subscribe('activeConversation:changed', (conv) => updateActive(conv));
      store.subscribe('messages:changed', (payload) => {
        if (!activeConv) return;
        if (!payload || payload.conversationId === activeConv.id) refreshMessages();
      });
      store.subscribe('messages:external', () => { if (activeConv) refreshMessages(); });
      store.subscribe('offers:changed', (payload) => {
        if (!activeConv) return;
        if (!payload || payload.conversationId === activeConv.id) refreshMessages();
      });
      store.subscribe('offers:external', () => { if (activeConv) refreshMessages(); });
      render();
    },
