...
export function createOfferCard({ store, offerService, profileService }) {
  return async function renderOfferCard(
    message,
    { mine, avatarUrl, currentRole, onUpdate },
  ) {
    ...
    let offer = null;
    try {
      offer = await offerService.getOffer(message.content);
    } catch {}
    ...
    msgMeta.textContent = new Date(message.timestamp).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
