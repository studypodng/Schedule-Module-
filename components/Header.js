...
export function createHeader({ store, profileService }) {
  ...
  function renderHeader() {
    ...
    ham.innerHTML = '☰';
    ham.addEventListener('click', () => store.publish('ui:toggleLeftMobile', true));
    ...
    if (otherProfile) {
      ...
      txt.textContent = otherProfile.online ? 'online' : formatOffline(otherProfile.lastOfflineAt);
    ...
    reset.addEventListener('click', () => store.publish('ui:requestReset', true));
  ...
  async function updateActiveConversation(conv) {
    activeConv = conv;
    if (!conv) { otherProfile = null; renderHeader(); return; }
    const state = store.getState();
