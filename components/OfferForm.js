...
export function createOfferForm({ store, offerService }) {
  ...
  function renderForm() {
    ...
    const fields = [
      ...
      { name:'time', label:'Time', type:'text', placeholder:'e.g. 10:00 AM - 11:30 AM', full:false },
    ...
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const data = {};
      for (const [k, { input }] of Object.entries(inputs)) data[k] = input.value;
  ...
  return {
    ...
    mount() {
      ...
      store.subscribe('ui:editOffer', (offer) => {
        ...
        const convs = JSON.parse(localStorage.getItem('estudy_conversations')||'[]');
