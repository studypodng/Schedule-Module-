...
export function createLeftNav({
  ...
  async function refresh() {
    ...
    const ids = conversations.map((c) =>
      state.currentRole === "tutor" ? c.studentId : c.tutorId,
    );
    const unique = [...new Set(ids)];
    const profs = await Promise.all(
      unique.map((id) => profileService.getProfile(id)),
    );
  ...
  function render() {
    ...
    toggle.addEventListener("click", () => {
      ...
      document.dispatchEvent(
        new CustomEvent("leftCollapseChanged", { detail: !isCollapsed }),
      );
    });
    ...
    if (conversations.length === 0) {
      ...
      for (const conv of conversations) {
        ...
        item.setAttribute(
          "aria-label",
          `Open conversation with ${profile?.name || "Unknown"}`,
        );
        ...
        delBtn.setAttribute(
          "aria-label",
          `Delete conversation with ${profile?.name || ""}`,
        );
        delBtn.textContent = "✕";
